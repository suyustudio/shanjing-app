: HEARTBEAT.md - 定期检查清单

## 检查频率
每 10 分钟检查一次

## 收到 Heartbeat 后必须执行（不能只回复 HEARTBEAT_OK）

### 1. 立即检查 GitHub Actions 状态
- [x] 执行：`curl -s "https://api.github.com/repos/suyustudio/shanjing-app/actions/runs?per_page=3"`
- [x] 记录构建编号、状态、结论
- [x] 如有失败，分析原因

### 2. 检查 Push 状态
- [x] 执行：`git status | grep "ahead"`
- [x] 如有未 push 的 commit，立即重试
- [x] 记录 push 结果

### 3. 检查 Memory 记录
- [x] 检查今日 memory 文件是否存在
- [x] 如有未记录的工作内容，立即写入
- [x] **不能只在聊天说"记住了"，必须落盘**

### 4. 主动汇报
- [x] 构建失败 → 告知用户
- [x] 需要操作 → 明确说明
- [x] push 失败 → 请求协助

## 记忆锚点
- **HEARTBEAT_OK ≠ 工作完成**
- **收到 heartbeat = 开始工作**
- **不检查 = 失职**
- **聊天说记住了 ≠ 写入文件**

---

## 当前状态（2026-03-20 晚上 7:27）

### ✅ 本地无未推送提交，构建状态正常

**Git 状态：**
- 本地分支：与 origin/main 同步 ✅
- 未推送提交：无 ✅
- 未跟踪文件：`shanjing-test/repo/`（测试目录，无需提交）
- 未暂存修改：`HEARTBEAT.md`（当前更新）

**GitHub Actions：**
| Build | 工作流 | 状态 | 说明 |
|-------|--------|------|------|
| **#166** | Build APK | ✅ success | 最新构建成功 |
| **#165** | Build APK | ✅ success | 上一构建成功 |
| #45 | APK Pre-check | ❌ failure | 辅助工作流，非阻塞 |
| #44 | APK Pre-check | ❌ failure | 辅助工作流，非阻塞 |

**关键信息：**
- ✅ 本地与远程同步，无需 push
- ✅ 主构建工作流（Build APK）全部成功
- ⚠️ APK Pre-check 辅助工作流持续失败（非阻塞）

**下一步：**
- [ ] 继续监控 Build #167+ 状态
- [ ] 关注 M5/M6 代码提交进度

---

## 历史状态存档
- 2026-03-20 19:27: 本地无未推送提交，Build #166 成功
- 2026-03-20 19:22: 本地无未推送提交，Build #166 成功
- 2026-03-20 17:22: 已推送 1 个提交，Build #161 排队中
- 详细历史见 `memory/2026-03-20.md`
