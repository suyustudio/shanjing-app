: HEARTBEAT.md - 定期检查清单

## 检查频率
每 10 分钟检查一次

## 收到 Heartbeat 后必须执行（不能只回复 HEARTBEAT_OK）

### 1. 立即检查 GitHub Actions 状态
- [x] 执行：`curl -s "https://api.github.com/repos/suyustudio/shanjing-app/actions/runs?per_page=3"`
- [x] 记录构建编号、状态、结论
- [x] 如有失败，分析原因

### 2. 检查 Push 状态
- [x] 执行：`git status | grep "ahead"`
- [x] 如有未 push 的 commit，立即重试
- [x] 记录 push 结果

### 3. 检查 Memory 记录
- [x] 检查今日 memory 文件是否存在
- [x] 如有未记录的工作内容，立即写入
- [x] **不能只在聊天说"记住了"，必须落盘**

### 4. 主动汇报
- [x] 构建失败 → 告知用户
- [x] 需要操作 → 明确说明
- [x] push 失败 → 请求协助

## 记忆锚点
- **HEARTBEAT_OK ≠ 工作完成**
- **收到 heartbeat = 开始工作**
- **不检查 = 失职**
- **聊天说记住了 ≠ 写入文件**

---

## 当前状态（2026-03-20 下午 5:22）

### ✅ 推送最后的 memory 更新完成

**Git 状态：**
- 本地分支：已推送 1 个提交到 origin/main ✅
- 提交哈希：`33f7bd19..c6f1fe27`
- 推送时间：2026-03-20 17:22

**构建状态：**
| Build | 工作流 | 状态 | 说明 |
|-------|--------|------|------|
| **#161** | Build APK | ⏳ 排队中 | 本次推送触发的新构建 |
| **#27** | 测试工作流 | ⏳ 排队中 | 辅助工作流 |
| #160 | Build APK with Debug | 🔄 进行中 | 之前的构建 |

**关键信息：**
- ✅ 成功推送 1 个本地提交（prisma schema 更新、memory 记录）
- ⚠️ 有未跟踪文件（M5成就系统徽章/设计资源/shanjing-test/repo）
- ⏳ Build #161 和 #27 正在排队等待执行

**下一步：**
- [ ] 监控 Build #161 执行结果
- [ ] 如构建失败，立即分析原因
- [ ] 需要时添加未跟踪文件到版本控制

---

## 历史状态存档
- 2026-03-20 17:10: 已推送 2 个提交，构建 #159/#25 排队中
- 详细历史见 `memory/2026-03-20.md`